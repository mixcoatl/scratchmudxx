...
using WeakEditorPtr = std::weak_ptr<Editor>;

//! Returns a copy of the weak Editor handle stored in userdata.
//! \param L the \c lua_State
//! \param index the stack index of the userdata
static WeakEditorPtr CheckWeakEditorPtr(
	lua_State* L,
	const int index) {
    return *static_cast<WeakEditorPtr*>(
	luaL_checkudata(L, index, EditorBindings::MetaName));
}

//! Handles Editor userdata garbage collection.
static int EditorGc(lua_State* L) {
    static_cast<WeakEditorPtr*>(
	luaL_checkudata(L, 1, EditorBindings::MetaName))->~WeakEditorPtr();
    return 0;
}
...
EditorPtr EditorBindings::Check(
	lua_State* L,
	const int index) {
    EditorPtr editor = CheckWeakEditorPtr(L, index).lock();
    if (!editor)
	luaL_argerror(L, index, "invalid editor");
    return editor;
}
...
static void RegisterEditorMeta(lua_State* L) {
    luaL_newmetatable(L, EditorBindings::MetaName);
    lua_pushvalue(L, -1);
