...
//! Clears the sector edit draft.
void Descriptor::ClearEditSector() {
    if (this->IsEditorActive())
	return;
    editSector_.reset();
    editName_.clear();
...
//! Sets the sector edit draft.
//! \param editSector the source sector, or null for a blank draft
SectorPtr Descriptor::SetEditSector(const SectorPtr& editSector) {
    if (this->IsEditorActive())
	return nullptr;
    const auto originalName = editSector ? editSector->GetName() : String();
    editSector_ = editSector ?
	std::make_shared<Sector>(*editSector) :
...
const char *Descriptor::GetColor(const int color) const noexcept {
    ...
    case Color::C_DAYTIME:		return this->GetColor(Color::C_AMBER);
    case Color::C_DESCRIPTION:		return this->GetColor(Color::C_OCHRE);
    case Color::C_EMPHASIS:		return this->GetColor(Color::C_LIME);
    case Color::C_ENUM:			return this->GetColor(Color::C_PURPLE);
    case Color::C_EXIT:			return this->GetColor(Color::C_AZURE);
    case Color::C_FAILED:		return this->GetColor(Color::C_CRIMSON);
    case Color::C_KEY:			return this->GetColor(Color::C_CRIMSON);
    case Color::C_MOVEMENT:		return this->GetColor(Color::C_AZURE);
    ...
    case Color::C_NIGHTTIME:		return this->GetColor(Color::C_AZURE);
    case Color::C_NOTICE:		return this->GetColor(Color::C_FOREST);
    case Color::C_PERCENT:		return this->GetColor(Color::C_CRIMSON);
    case Color::C_PEEK:			return this->GetColor(Color::C_TEAL);
    ...
    case Color::C_TITLE:		return this->GetColor(Color::C_LIME);
...
void Descriptor::SetCharacter(const InstancePtr& instance) noexcept {
	...
	if (player)
	    game_.GetPlayers()->Save(player->GetName());
...
void Descriptor::WritePrompt() {
    ...
    String label;
    if (instance_) {
	const auto player = instance_->GetPlayer();
	if (player && !player->GetName().empty())
	    label = player->GetName();
    }
    if (label.empty() && user_ && !user_->GetName().empty())
	label = user_->GetName();
    if (label.empty())
	label = GetName();

    char message[MaxString] = {'\0'};
    std::snprintf(message, sizeof(message), "%s%s%s>%s ",
	this->GetColor(Color::C_PROMPT),
	label.c_str(),
	this->GetColor(Color::C_PUNCTUATION),
