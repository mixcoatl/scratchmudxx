...
bool Game::PruneWorld(const WorldPtr& world, const bool force) noexcept {
	...
	for (const auto& instance: toEject) {
		...
		if (auto sourceWorld = instance->GetWorld())
	    ...
	    if (!start) {
		LOGGER_WORLD() << "Force prune: no start room for "
		    << (player ? player->GetName() : String()) << '.';
		if (auto descriptor = instance->GetDescriptor())
		    descriptor->Close();
		if (auto sourceWorld = instance->GetWorld())
		    sourceWorld->RemoveInstance(instance);
		continue;
	    }
	    if (!instance->MoveTo(start)) {
		LOGGER_WORLD() << "Force prune: MoveTo failed for "
		    << (player ? player->GetName() : String()) << '.';
		continue;
	    }
	}
