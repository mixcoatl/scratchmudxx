...
void Game::SetShutdown(const bool shutdown) noexcept {
	...
	// Close all descriptors before stopping
	// I/O context so pending operations cancel
	// cleanly.
	for (auto d: this->GetDescriptors())
	    d->Close();

	// Close() defers EraseDescriptor via post; clear now so the map
	// is empty even if stop() discards those posts.
	descriptors_.clear();
	ioContext_.stop();
    }
}
