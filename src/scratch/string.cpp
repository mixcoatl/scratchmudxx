...
String Strings::FormatDuration(
	const boost::chrono::steady_clock::duration& duration) {
    using boost::chrono::seconds;

    auto remaining = boost::chrono::duration_cast<seconds>(duration).count();
    if (remaining < 0)
	remaining = 0;

    std::vector<String> parts;
    const auto days = remaining / (24 * 60 * 60);
    remaining -= days * 24 * 60 * 60;
    const auto hours = remaining / (60 * 60);
    ...
    if (minutes)
	parts.push_back(std::to_string(minutes) + 'm');
    if (remaining || parts.empty())
	parts.push_back(std::to_string(remaining) + 's');
    return boost::algorithm::join(parts, " ");
}

//! Parses a duration string.
//! \param text the duration text
//! \param duration the parsed duration
//! \return \c true if \a text was parsed
//! \sa #FormatDuration(const boost::chrono::steady_clock::duration&)
bool Strings::ParseDuration(
	const String& text,
	boost::chrono::steady_clock::duration& duration) {
    std::basic_istringstream<typename String::value_type> iss(text);
    boost::chrono::seconds total{0};
	...
	case 'd': total += boost::chrono::hours(24 * value); break;
	case 'h': total += boost::chrono::hours(value); break;
	case 'm': total += boost::chrono::minutes(value); break;
	case 's': total += boost::chrono::seconds(value); break;
