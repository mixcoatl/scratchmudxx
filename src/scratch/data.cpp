...
//!     cannot be resolved
//! \return the data node at the specified key, or the default value
DataPtr Data::Get(
	const String& key,
	const DataPtr& defaultValue) noexcept {
    if (key.length() == 0)
...
//! \param defaultValue the value to return if the specified key
//!     cannot be resolved
//! \return the string map at the specified key, or the default value
StringMapCi<String> Data::GetStringMap(
	const String& key,
	const StringMapCi<String>& defaultValue) noexcept {
    DataPtr node = key.length() ? this->Get(key) : nullptr;
    if (key.length() && !node)
	return defaultValue;
    Data& dataRef = node ? *node : *this;
    StringMapCi<String> result;
    for (const auto& entry: dataRef.entries_)
	result[entry.first] = dataRef.GetString(entry.first);
    return result;
}
...
//! \param defaultValue the value to return if the specified key
//!     cannot be resolved
//! \return the string set at the specified key, or the default value
StringSetCi Data::GetStringSet(
	const String& key,
	const StringSetCi& defaultValue) noexcept {
    DataPtr node = key.length() ? this->Get(key) : nullptr;
    if (key.length() && !node)
	return defaultValue;
    Data& dataRef = node ? *node : *this;
    StringSetCi result;
    for (const auto& entry: dataRef.entries_) {
	const auto value = dataRef.GetString(entry.first);
	if (!value.empty())
	    result.insert(value);
    }
    return result;
...
bool Data::GetYesNo(
	...
	if (!Strings::CompareCi(found, "Y") || !Strings::CompareCi(found, "Yes"))
	    return true;

	// Check for 'No' values.
	if (!Strings::CompareCi(found, "N") || !Strings::CompareCi(found, "No"))
	    return false;
    }
    return defaultValue;
}
...
DataPtr Data::PutFormattedVa(
	const String& key,
	const char* format, std::va_list args) noexcept {
    return this->PutString(key, Strings::FormatVa(format, args));
}
...
//! \return the new or updated data node, or \c nullptr
DataPtr Data::PutStringMap(
	const String& key,
	const StringMapCi<String>& value) noexcept {
    DataPtr node = key.length() ? this->Get(key) : nullptr;
    if (key.length() && !node) {
	node = std::make_shared<Data>();
	if (this->Put(key, node) != node)
	    return DataPtr();
    }
    Data& dataRef = node ? *node : *this;
    dataRef.Clear();
    for (const auto& entry: value)
	dataRef.PutString(entry.first, entry.second);
    return node;
}
...
//! \return the new or updated data node, or \c nullptr
DataPtr Data::PutStringSet(
	const String& key,
	const StringSetCi& value) noexcept {
    DataPtr node = key.length() ? this->Get(key) : nullptr;
    if (key.length() && !node) {
	node = std::make_shared<Data>();
	if (this->Put(key, node) != node)
...
void Data::ReadStringBlock(std::istream& inStream) noexcept {
	...
	// Special handling for tilde character.
	    ...
	    if (inStream.peek() != '~') {
		if (oss.tellp() > 0) {
		    const auto partial = oss.str();
		    if (partial.find_first_not_of(" \t") != String::npos)
	...
	if (index == String::npos)
	    continue; // Ignore entirely blank lines.
