...
int Data::GetFormattedVa(
    ...
    if (found && found->mValue.length()) {
	return std::vsscanf(found->mValue.c_str(), format, args);
    }
    return -1;
}
...
String Data::GetString(
    ...
    return result && result->mValue.length() ? result->mValue : defaultValue;
}
...
DataPtr Data::PutString(
    ...
    return this->PutString(key, value && *value != '\0' ? value : "");
}
...
void Data::WriteString(
    ...
    auto begin = mValue.cbegin(), end = mValue.cend();
    if (mValue.find('\n') != String::npos) {
...
void Data::WriteStructureEntry(
    ...
    } else if (entry.second->mValue.find('\n') != String::npos) {
	outStream << '-' << std::endl;
