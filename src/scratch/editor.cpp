...
std::vector<String> TruncateRows(
	...
	if (boost::algorithm::join(candidate, "\r\n").size() > maxLength)
	    break;
	kept.swap(candidate);
    }
    return kept;
...
std::size_t Editor::GetLength() const noexcept {
    return boost::algorithm::join(lines_, "\r\n").size();
}
...
bool Editor::Dispatch(
	...
	const String& args) {
    if (Scratch::Algorithm::Strings::CompareCi(command, "abort") == 0)
	return EditorVerbs::Abort(*this);
    if (Scratch::Algorithm::Strings::CompareCi(command, "clear") == 0)
	return EditorVerbs::Clear(*this);
    if (Scratch::Algorithm::Strings::CompareCi(command, "delete") == 0)
	return EditorVerbs::Delete(*this, args);
    if (Scratch::Algorithm::Strings::CompareCi(command, "help") == 0)
	return EditorVerbs::Help(*this);
    if (Scratch::Algorithm::Strings::CompareCi(command, "list") == 0)
	return EditorVerbs::List(*this);
    if (Scratch::Algorithm::Strings::CompareCi(command, "listn") == 0)
	return EditorVerbs::Listn(*this);
    if (Scratch::Algorithm::Strings::CompareCi(command, "save") == 0)
	return EditorVerbs::Save(*this);
    if (Scratch::Algorithm::Strings::CompareCi(command, "wrap") == 0)
...
bool Editor::Wrap(const std::size_t indent) {
    ...
    if (boost::algorithm::join(wrapped, "\r\n").size() > maxLength_) {
