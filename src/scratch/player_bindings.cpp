...
using WeakPlayerRepositoryPtr = std::weak_ptr<PlayerRepository>;

static WeakPlayerRepositoryPtr CheckWeakPlayerRepositoryPtr(
	lua_State* L,
	const int index = 1);

//! Returns a copy of the weak Player handle stored in userdata.
//! \param L the \c lua_State
//! \param index the stack index of the userdata
static WeakPlayerPtr CheckWeakPlayerPtr(
	lua_State* L,
	const int index) {
    return *static_cast<WeakPlayerPtr*>(
	luaL_checkudata(L, index, PlayerBindings::MetaName));
}

//! Handles Player userdata garbage collection.
static int PlayerGc(lua_State* L) {
    static_cast<WeakPlayerPtr*>(
	luaL_checkudata(L, 1, PlayerBindings::MetaName))->~WeakPlayerPtr();
    return 0;
}

//! Returns whether \p name is a member of enumeration \p id.
static bool EnumerationHasMember(
	Scratch::Core::Game& game,
	const char* id,
	const String& name) {
    auto enumeration = game.GetEnumerations()->Get(id);
    if (!enumeration)
	return false;
    return enumeration->Contains(name);
}

//! Reads a Lua string array at \p index into \p out.
static void CheckStringSetTable(
	lua_State* L,
	const int index,
	StringSetCi& out) {
    luaL_checktype(L, index, LUA_TTABLE);
    out.clear();
    const int len = static_cast<int>(luaL_len(L, index));
    for (int i = 1; i <= len; ++i) {
	lua_rawgeti(L, index, i);
	if (!lua_isstring(L, -1)) {
	    lua_pop(L, 1);
	    out.clear();
	    luaL_argerror(L, index, "members must be strings");
	}
	out.insert(Lua::CheckString(L, -1));
	lua_pop(L, 1);
    }
}

//! Handles Player:add_permission(permission).
static int PlayerAddPermission(lua_State* L) {
    if (lua_gettop(L) != 2)
    ...
    if (!EnumerationHasMember(game, "Permission", name)) {
	player.reset();
	return luaL_argerror(L, 2, "unknown permission");
    }
    player->AddPermission(name);
    return 0;
}
...
static int PlayerAddPreference(lua_State* L) {
    ...
    auto player = PlayerBindings::Check(L, 1);
    const auto name = Lua::CheckString(L, 2);
    if (!EnumerationHasMember(game, "Preference", name)) {
	player.reset();
...
//! Handles Player:erase_permission(permission).
static int PlayerErasePermission(lua_State* L) {
    if (lua_gettop(L) != 2)
	return luaL_error(L, "erase_permission expects 1 argument");
    luaL_checktype(L, 2, LUA_TSTRING);
    PlayerBindings::Check(L, 1)->ErasePermission(Lua::CheckString(L, 2));
    return 0;
}
...
//! Handles Player:get_permissions().
static int PlayerGetPermissions(lua_State* L) {
    auto& lua = Lua::CheckLua(L);
    auto player = PlayerBindings::Check(L, 1);
    auto permissions = player->GetPermissions();
    player.reset();
...
//! Handles Player:has_permission(permission).
static int PlayerHasPermission(lua_State* L) {
    if (lua_gettop(L) != 2)
	return luaL_error(L, "has_permission expects 1 argument");
    luaL_checktype(L, 2, LUA_TSTRING);
    auto& lua = Lua::CheckLua(L);
    auto player = PlayerBindings::Check(L, 1);
    const bool present = player->HasPermission(Lua::CheckString(L, 2));
    player.reset();
...
//! Handles Player:set_permissions(table).
static int PlayerSetPermissions(lua_State* L) {
    if (lua_gettop(L) != 2)
	return luaL_error(L, "set_permissions expects 1 argument");
    auto& game = Lua::CheckGame(L);
    StringSetCi permissions;
    CheckStringSetTable(L, 2, permissions);
    ...
    PlayerBindings::Check(L, 1)->SetPermissions(permissions);
    return 0;
}
...
static int PlayerSetPreferences(lua_State* L) {
    ...
    CheckStringSetTable(L, 2, preferences);
...
PlayerPtr PlayerBindings::Check(
	lua_State* L,
	const int index) {
    PlayerPtr player = CheckWeakPlayerPtr(L, index).lock();
    if (!player)
	luaL_argerror(L, index, "invalid player");
    return player;
}
...
static void RegisterPlayerMeta(lua_State* L) {
    luaL_newmetatable(L, PlayerBindings::MetaName);
    lua_pushvalue(L, -1);
    lua_setfield(L, -2, "__index");

    static const luaL_Reg methods[] = {
	{"__gc", PlayerGc},
	{"add_permission", PlayerAddPermission},
	{"add_preference", PlayerAddPreference},
	{"erase_permission", PlayerErasePermission},
	...
	{"get_permissions", PlayerGetPermissions},
	{"get_preferences", PlayerGetPreferences},
	{"has_permission", PlayerHasPermission},
	...
	{"set_permissions", PlayerSetPermissions},
...
PlayerRepository& PlayerBindings::CheckRepository(
	lua_State* L,
	const int index) {
    PlayerRepositoryPtr repo = CheckWeakPlayerRepositoryPtr(L, index).lock();
    if (!repo)
	luaL_argerror(L, index, "invalid player repository");
    return *repo;
}

//! Returns a copy of the weak PlayerRepository handle stored in userdata.
//! \param L the \c lua_State
//! \param index the stack index of the userdata
static WeakPlayerRepositoryPtr CheckWeakPlayerRepositoryPtr(
	lua_State* L,
	const int index) {
    return *static_cast<WeakPlayerRepositoryPtr*>(
	luaL_checkudata(L, index, PlayerBindings::RepositoryMetaName));
}

//! Handles PlayerRepository:clear().
static int PlayerRepositoryClear(lua_State* L) {
    if (lua_gettop(L) != 1)
	return luaL_error(L, "clear expects no arguments");
    PlayerBindings::CheckRepository(L).Clear();
    return 0;
}
...
static int PlayerRepositoryGc(lua_State* L) {
    static_cast<WeakPlayerRepositoryPtr*>(
	luaL_checkudata(L, 1, PlayerBindings::RepositoryMetaName))->~WeakPlayerRepositoryPtr();
    return 0;
}
...
static void RegisterPlayerRepositoryMeta(lua_State* L) {
    luaL_newmetatable(L, PlayerBindings::RepositoryMetaName);
    lua_pushvalue(L, -1);
    lua_setfield(L, -2, "__index");

    static const luaL_Reg methods[] = {
	{"__gc", PlayerRepositoryGc},
	{"clear", PlayerRepositoryClear},
