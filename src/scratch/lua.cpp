...
//! Registry key for loaded project modules.
static char moduleCacheRegistryKey;

//! Project Lua module root.
static const char moduleRoot[] = "lua";
...
Lua::Lua(Game& game) :
    ...
    lua_newtable(lua_);
    lua_pushlightuserdata(lua_, &moduleCacheRegistryKey);
    lua_pushvalue(lua_, -2);
    lua_settable(lua_, LUA_REGISTRYINDEX);
    lua_pop(lua_, 1);

    this->Function("require", &Lua::Require);
...
//! Resolves a project Lua module path.
//! \param moduleName the module name
//! \return the resolved path, or empty when unavailable
//! \sa #Require(const String&)
String Lua::GetModulePath(const String& moduleName) const {
    if (moduleName.empty() || moduleName.front() == '.' ||
	moduleName.back() == '.' ||
	moduleName.find("..") != String::npos)
	return String();
    const auto invalid = std::find_if_not(
	moduleName.begin(), moduleName.end(), [](const char c) {
	    return std::isalnum(static_cast<unsigned char>(c)) ||
		c == '_' || c == '.';
	});
    if (invalid != moduleName.end())
	return String();

    String relative = moduleName;
    boost::replace_all(relative, ".", "/");

    boost::system::error_code error;
    const auto root = boost::filesystem::canonical(moduleRoot, error);
    if (error)
	return String();
    const auto module = boost::filesystem::canonical(
	root / (relative + ".lua"), error);
    if (error || !boost::filesystem::is_regular_file(module))
	return String();

    const auto relativePath = boost::filesystem::relative(
	module, root, error);
...
//! Loads a project Lua module.
//! \param moduleName the module name
//! \return the module value
//! \sa #GetModulePath(const String&) const
LuaResult Lua::Require(const String& moduleName) {
    return LuaResult([this, moduleName](lua_State* L) {
	const String path = GetModulePath(moduleName);
	if (path.empty())
	    return luaL_error(L, "invalid or unavailable Lua module");

	lua_pushlightuserdata(L, &moduleCacheRegistryKey);
	lua_gettable(L, LUA_REGISTRYINDEX);
	const int cache = lua_absindex(L, -1);
	lua_getfield(L, cache, moduleName.c_str());
	if (!lua_isnil(L, -1)) {
	    lua_remove(L, cache);
	    return 1;
	}
	...
	if (!file) {
	    lua_pop(L, 1);
	    return luaL_error(L, "Lua module cannot be opened");
	}
	...
	if (!lua_setupvalue(L, -2, 1)) {
	    lua_pop(L, 1);
	    return luaL_error(L, "Lua module has no environment");
	}
	...
	lua_pushvalue(L, -1);
	lua_setfield(L, cache, moduleName.c_str());
	lua_remove(L, cache);
	return 1;
    });
}
