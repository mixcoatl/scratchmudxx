...
String ResolveProperty(
    ...
    const auto direction = param.ToDirection();
    if (Scratch::Algorithm::Strings::CompareCi(prop, "From") == 0) {
	if (!Direction::IsDefined(direction))
	    return "<Invalid>";
	return boost::algorithm::to_lower_copy(Direction::ToString(
	    Direction::Opposite(direction)));
    }
    if (Scratch::Algorithm::Strings::CompareCi(prop, "FromPhrase") == 0) {
	if (!Direction::IsDefined(direction))
	    return "<Invalid>";
	return Direction::ToPhrase(Direction::Opposite(direction));
    }
    if (Scratch::Algorithm::Strings::CompareCi(prop, "To") == 0) {
	if (!Direction::IsDefined(direction))
	    return "<Invalid>";
	return boost::algorithm::to_lower_copy(
	    Direction::ToString(direction));
    }
    if (Scratch::Algorithm::Strings::CompareCi(prop, "ToPhrase") == 0) {
	if (!Direction::IsDefined(direction))
	    return "<Invalid>";
	return Direction::ToPhrase(direction);
    }
    return "<Invalid>";
}
