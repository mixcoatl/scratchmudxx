...
static int DescriptorPrintColumns(lua_State* L) {
    ...
    cells.reserve(static_cast<std::size_t>(std::max(0, len)));
    for (int i = 1; i <= len; ++i) {
	lua_rawgeti(L, 2, i);
	if (!lua_isstring(L, -1)) {
	    lua_pop(L, 1);
	    cells.clear();
	    d.reset();
	    return luaL_argerror(L, 2, "string array expected");
	}
	cells.push_back(Lua::CheckString(L, -1));
	lua_pop(L, 1);
    }
