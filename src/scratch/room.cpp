...
Room::Room() noexcept :
	...
	exits_(),
	immortalBit_(false),
	lighting_(Lighting::LIGHTING_NATURAL),
	occupantLimit_(),
	overrideIndoors_(),
	privateBit_(false),
	sector_(),
...
Room::Room(const Room& other) noexcept :
	...
	occupantLimit_(other.occupantLimit_),
	overrideIndoors_(other.overrideIndoors_),
	privateBit_(other.privateBit_),
...
void Room::MaterializeInto(const WorldPtr& world) {
    ...
    SectorPtr sector;
    if (!sector_.empty()) {
	auto sectors = world->GetGame().GetSectors();
	if (sectors)
	    sector = sectors->Get(sector_);
    }

    auto instance = std::make_shared<Instance>();
    instance->SetName(Strings::GenerateCopy());
    instance->SetZone(this->GetZone());
    auto specials = std::make_shared<RoomSpecials>(room, sector);
    for (const auto& entry: exits_) {
	if (entry.second)
	    entry.second->MaterializeInto(specials);
    }
...
void Room::ReadData(const DataPtr& data) noexcept {
    ...
    sector_ = data->GetString("Sector");

    if (data->Get("Indoors", nullptr))
	overrideIndoors_ = data->GetYesNo("Indoors");

    if (data->Get("OccupantLimit", nullptr)) {
	occupantLimit_ = static_cast<std::uint8_t>(
	    data->GetNumber("OccupantLimit"));
    } else {
	occupantLimit_.reset();
    }

    const auto lightingName = data->GetString("Lighting");
    if (!lightingName.empty()) {
	const auto lighting = Lighting::ByName(lightingName);
	if (Lighting::IsDefined(lighting))
	    lighting_ = lighting;
    }
...
void Room::ReadRoomBitsData(const DataPtr& data) noexcept {
    ...
    privateBit_ = data->GetYesNo("Private");
    if (data->GetYesNo("Tunnel") && !occupantLimit_)
	occupantLimit_ = 1;
}
...
void Room::WriteData(const DataPtr& data) const noexcept {
    ...
    if (!sector_.empty())
	data->PutString("Sector", sector_);

    if (overrideIndoors_)
	data->PutYesNo("Indoors", *overrideIndoors_);

    if (occupantLimit_)
	data->PutNumber("OccupantLimit", *occupantLimit_);

    if (lighting_ != Lighting::LIGHTING_NATURAL) {
	const auto lighting = Lighting::ToString(lighting_);
	if (!lighting.empty())
	    data->PutString("Lighting", lighting);
    }
