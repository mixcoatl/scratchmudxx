...
public:
	...
	const auto stem = Scratch::Algorithm::StringSanitizeCopy(thingId);
	...
	LOGGER_STORAGE() << "Loaded " << nThingBytes << " byte(s) in " << out.size()
			 << " " << this->GetLabel() << "(s).";
	return true;
    }
	...
	for (const auto& pair : in) {
		...
		return false;
	    ++nThings;
	    const auto stem = Scratch::Algorithm::StringSanitizeCopy(pair.first);
	    if (!stem.empty())
