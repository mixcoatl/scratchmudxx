...
public:
    ...
    //! Adds an unnamed choices section.
    void AddChoices(const bool fold) {
	this->AddSection(fold, SectionKind::Choices);
    }

    //! Adds an unnamed listing section.
    void AddListing(const bool fold) {
	this->AddSection(fold, SectionKind::Listing);
    }

    //! Adds a named choices section.
    void AddNamedChoices(const String& title, const bool fold) {
	this->AddSection(fold, SectionKind::Choices, title);
    }

    //! Adds a named listing section.
    void AddNamedListing(const String& title, const bool fold) {
	this->AddSection(fold, SectionKind::Listing, title);
    }
    ...
    //! Returns the matching choices key, or no value.
    boost::optional<String> MatchKey(const String& line) const;
