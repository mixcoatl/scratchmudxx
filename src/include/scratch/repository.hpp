...
public:
    ...
    //! \sa #Get(const String&) const
    void Store(const String& thingId, const ThingPtr& thing) noexcept {
	if (!thing) {
	    LOGGER_ASSERT() << "Invalid `thing` ThingPtr.";
	} else if (Scratch::Algorithm::StringCompareCi(
		Scratch::Algorithm::StringSanitizeCopy(thingId), thingId)) {
	    ...
	    storage_.Write(thingId, canonical, things_);
	}
    }
